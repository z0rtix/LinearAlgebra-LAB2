...
def auc_manual(fpr, tpr):
    return np.trapezoid(tpr, fpr)   # было np.trapz, исправлено
...
def run_baseline(X_train, y_train, X_test, y_test, output_dir='results'):
    ...
    plot_decision_boundary(model, X_train, y_train, 'Baseline Decision Boundary (Train)',
                           save_path=f'{output_dir}/baseline_boundary_train.png')
    plot_decision_boundary(model, X_test, y_test, 'Baseline Decision Boundary (Test)',
                           save_path=f'{output_dir}/baseline_boundary_test.png')
    return model
...
def run_init_experiment(X_train, y_train, X_test, y_test, output_dir='results'):
    init_methods = {
        ...
        'small_random': lambda: (np.random.randn(2, 1) * 0.01, 0.0),
        'large_random': lambda: (np.random.randn(2, 1) * np.sqrt(10), 0.0)
    }
    results = {}
    plt.figure(figsize=(12, 4))
...
def run_custom_data_experiments(output_dir='results'):
    ...
    datasets = {
        'linear_separable': generate_linear_separable(n_samples=200),
        'xor': generate_xor(n_samples=200),
        'circle': generate_circle(n_samples=200, radius=1.0, noise=0.0)
    }
    ...
    for name, (X, y) in datasets.items():
        ...
        plot_decision_boundary(model, X_test_scaled, y_test, f'Decision Boundary: {name}',
                               save_path=f'{output_dir}/custom_{name}_boundary.png')

def run_loss_comparison(X_train, y_train, X_test, y_test, output_dir='results'):
    print("\n=== Loss Comparison: Cross-Entropy vs Hinge ===")
    models = {
        'cross_entropy': Perceptron(n_features=2, loss_type='cross_entropy'),
        'hinge': Perceptron(n_features=2, loss_type='hinge')
    }
...
def run_metrics_analysis(model, X_test, y_test, output_dir='results'):
    ...
    if np.any(errors):
        ...
        plt.scatter(X_test[errors, 0], X_test[errors, 1], c=y_test[errors], edgecolors='k', cmap='coolwarm', marker='x', s=100, linewidth=2, label='Error')
        plt.xlabel('Feature 1')
        plt.ylabel('Feature 2')
